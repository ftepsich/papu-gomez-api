...
/**
     ...
     */
turnoController.get = async (req, res, next) => {
    try {
        const turnos = await turnoSql.fetchAll();
        res.json(turnos);
    } catch(error){
...
/**
     ...
     * @param {CallableFunction} next
     */
turnoController.asignar = async (req, res, next) => {
    try {
        const {
            id_paciente,
            id_profesional,
        ...
        const turnos_asignados_profesional = await turnoSql.asignadoAProfesional(id_profesional);
        ...
        const turno = await turnoSql.create(id_paciente, id_profesional, practica, duracion);
        if( turno.rowCount == 0 ){
            throw ('No se pudo crear el turno');
        }
        const id_turno = turno.rows[0].id;
        const estado_turno = await turnoSql.asignar(id_turno, fecha_hora_turno);
